1 # CI Pipeline — GitHub Actions — Implementation Notes
       ...
       79
       80 **Fix applied:** Re-ran `dotnet format FeatureFlagService.sln --diagnostics IDE0011 IDE0008` after the rebase, then re-ran `dotnet csharpier format .` to reconcile any line-length disagreements introduced by `dotnet format`.
       81
       82 ---
       83
       84 ## Key Decisions
      ...
      101
      102 - `dotnet build FeatureFlagService.sln --no-incremental -p:TreatWarningsAsErrors=true` → **0 errors, 0 warnings**
      103 - `dotnet csharpier check .` → **41 files checked, 0 violations**
      104 - `dotnet test FeatureFlagService.sln --filter "Category!=Integration"` → **8/8 passing**
