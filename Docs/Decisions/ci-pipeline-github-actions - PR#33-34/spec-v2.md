...
| 4 | Restore packages | `dotnet restore Banderas.sln` | Hard fail |
| 5 | Format check | `dotnet csharpier --check .` | Hard fail — PR cannot merge |
| 6 | Build | `dotnet build Banderas.sln --no-restore --no-incremental -p:TreatWarningsAsErrors=true` | Hard fail — PR cannot merge |
...
| 1 | Checkout | `actions/checkout@v4` | Hard fail |
| 2 | Setup .NET | `actions/setup-dotnet@v4` — `dotnet-version: '10.x'`, `cache: 'nuget'` | Hard fail |
| 3 | Restore packages | `dotnet restore Banderas.sln` | Hard fail |
| 4 | Run unit tests | `dotnet test Banderas.sln --no-restore --filter "Category!=Integration"` | Hard fail — PR cannot merge |
...
All test classes in `Banderas.Tests` must carry a `[Trait]` attribute before this pipeline is activated. Claude Code must update existing test files as part of this work.
...
|---|---|---|---|
| 🔴 error | Banderas.Api/Controllers/BanderasController.cs | 42 | Domain entity `Flag` returned directly from service call — map to `FlagResponse` DTO |
| 🟡 warning | Banderas.Application/Services/BanderasService.cs | 88 | Missing CancellationToken propagation to repository call |
| 🔵 suggestion | Banderas.Domain/Entities/Flag.cs | 15 | Consider guard clause for null name on construction |
...
Project namespace roots: Banderas.Domain, Banderas.Application, Banderas.Infrastructure, Banderas.Api
...
> This fail-open behavior is appropriate for Phase 1 on a portfolio project. When Banderas moves toward production infrastructure (Phase 8+) and the AI reviewer is gating changes to security-critical or customer-facing code paths, this should be revisited. Options include: retry with exponential backoff before failing open, alerting via a GitHub issue comment, or switching to fail-closed for PRs targeting `main` only.
...
- [ ] All existing tests in `Banderas.Tests` carry `[Trait("Category", "Unit")]`
