...
- [X] **Convert `StrategyConfig` from raw `string` to typed Value Objects** — one Value Object per strategy type _(shipped as a single `StrategyConfig` Value Object with `ValidatedFor` discriminator + per-strategy validators registered via `StrategyConfigFactory`; invalid config caught at construction; PR TBD)_
- [X] **Enforce config/strategy type consistency inside `Flag`** — `Flag` ctor and `Reconfigure` reject a `StrategyConfig` whose `ValidatedFor` does not match the supplied strategy type; throws `FlagDomainException` (PR TBD)
- [X] **Add `Description` to `Flag` definition** — environment-agnostic metadata _(shipped on per-environment `Flag` row as `string?` with ≤500-char validation; will migrate to `FlagDefinition` aggregate when the split lands; see `Docs/Decisions/flag-description-and-tags/spec.md`, PR TBD)_
- [X] **Add `Tags` collection to `Flag` definition** — environment-agnostic organizational labels _(shipped on per-environment `Flag` row as `IReadOnlyList<string>` stored `jsonb`; normalized to lowercase + deduped; ≤20 entries × ≤50 chars, char-class `^[a-z0-9\-_]+$`; will migrate to `FlagDefinition` aggregate when the split lands; see `Docs/Decisions/flag-description-and-tags/spec.md`, PR TBD)_
- [X] **Introduce `Variation` as a Value Object on `Flag`** — possible return values defined once on the definition; `VariationKind` enum (Boolean | String | Number | Json); single-element and collection-level invariants enforced on `Flag`; `UpdateVariations` mutation; zero-downtime migration with Boolean menu backfill _(PR TBD)_
