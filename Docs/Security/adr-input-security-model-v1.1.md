...
[Pull Request #XX](https://github.com/amodelandme/FeatureFlagService/pull/xx)
...
`FeatureFlag.Application/Validators/`. Validators call it inside `Must()` lambdas.
`FeatureFlagService` calls it directly before evaluation. Same function, two call
...
`InputSanitizer` is an `internal static` class in `FeatureFlag.Application/Validators/`.
...
**Point 2 — Service layer:** `FeatureFlagService.IsEnabledAsync` rebuilds
...
- `InputSanitizer` is `internal` to `FeatureFlag.Application` — the Api project
...
*FeatureFlagService | Security ADR v1.1 — current*
